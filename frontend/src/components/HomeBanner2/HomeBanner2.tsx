...
const HomeBanner2 = () => {
    ...
    const fetchData = async () => {
      ...
      const response = await fetch('http://localhost:8080/plan/workouts', {
        method: 'GET'
      });
      if(!response.ok){
    ...
    useEffect(()=>{
        fetchWorkouts();//change to fetchdata
        }, [])
    ...
    return (
        <>
            ...
            <h2 className="mainhead1">Fitness</h2>
      ...
      >
       ...
       {workouts.length > 0 ? (  //map data instead of workouts change from workouts.map to data.map
        workouts.map((item: any, index: number)=> (
          <SwiperSlide key={index}>
            <div className="workout-card" onClick={()=>{handleCardClick(item.type)}}>
